...
import (
	...
	"jsls.work/ge/contracts/exchange"
)
...
func main() {
	...
	for _, vLog := range logs {
		...
		case logFillEvent.Hex():
			...
			fillEvent.Maker = common.HexToAddress(string(vLog.Topics[1].Hex()))
			fillEvent.FeeRecipient = common.HexToAddress(string(vLog.Topics[2].Hex()))
		...
		case logCancelEvent.Hex():
			...
			cancelEvent.Maker = common.HexToAddress(string(vLog.Topics[1].Hex()))
			cancelEvent.FeeRecipient = common.HexToAddress(string(vLog.Topics[2].Hex()))
		...
		case logErrorEvent.Hex():
			...
			fmt.Println("Error ID: %d\n", errorEvent.ErrorID)
			fmt.Println("Order Hash: %s\n", hexutil.Encode(errorEvent.OrderHash[:]))
		}
	}
}
